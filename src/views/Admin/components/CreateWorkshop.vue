  ...
  >
    <div class="popup">
      <div class="popup-inner">
        <h2 style="color: black; text-align: center">Tạo Workshop</h2>
        <form @submit="handleSubmit">
          ...
          <div class="form-group">
            ...
            <input type="number" id="attendees" v-model="attendees" />
          </div>
          <div class="form-group">
            <input
              type="text"
            ...
            <br />
            <label for="title">Các mentors chọn</label>
            <br />
            <button
              @click="resetMentor()"
              style="
            ...
            <pre>
                  <div
                          ...
                          >
                            <a
                              :href="
                                'https://kim-bk.github.io/view-mentor/' + mentor.id
                              "
                            >
          ...
          <button type="submit" @click="createWorkshop()">Tạo</button>
          <button
            class="btn-close"
            @click="$router.go(-1)"
...
export default {
  ...
  methods: {
    handleSelectChange() {
      ...
      if (
        !this.selectedMentors.some((mentor) => mentor.id === selectedOption)
      ) {
        var selectedMentor = this.getMentors.find(
          (mentor) => mentor.id === selectedOption
        );
      ...
      console.log(
        "Selected option:",
        this.selectedMentors.map((mentor) => mentor.id).join(", ")
      );
    },
    ...
    areAllPropertiesNotNull(formData) {
      for (const [key, value] of formData.entries()) {
        if (value === null || value === undefined || value === "") {
          console.log(`Data for key '${key}' is null or empty.`);
          return false;
        }
      }
      return true;
    ...
    async createWorkshop() {
      this.mentorIds = this.selectedMentors
      ...
      if (this.areAllPropertiesNotNull(formData)) {
        if (this.canProcess) {
        ...
        this.errorM = "Thiếu thông tin vui lòng kiểm tra lại!";
        this.canProcess = false;
      }
    },
  },
